...
static std::map<std::string, unsigned int> mapFlagNames = {
    ...
    {std::string("ENABLE_SIGHASH_FORKID_SHIFT"), (unsigned int)SCRIPT_ENABLE_SIGHASH_FORKID_SHIFT},
};
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
	    ...
	    for (unsigned int inpIdx = 0; inpIdx < inputs.size(); inpIdx++) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++)
            {
                ...
                unsigned int verify_flags = ParseScriptFlags(test[2].get_str()) | SCRIPT_ENABLE_SIGHASH_FORKID | SCRIPT_ENABLE_SIGHASH_FORKID_SHIFT;
...
void CreateCreditAndSpend(const CKeyStore& keystore, const CScript& outscript, CTransactionRef& output, CMutableTransaction& input, bool success = true)
{
    ...
    bool ret = SignSignature(keystore, *output, inputm, 0, SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT);
    assert(ret == success);
...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction) {
    ...
    sigHashes.push_back(SIGHASH_NONE | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT | SIGHASH_ANYONECANPAY);
    sigHashes.push_back(SIGHASH_SINGLE | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT | SIGHASH_ANYONECANPAY);
    sigHashes.push_back(SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT | SIGHASH_ANYONECANPAY);
    sigHashes.push_back(SIGHASH_NONE | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT);
    sigHashes.push_back(SIGHASH_SINGLE | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT);
    sigHashes.push_back(SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT);

...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, TestChain100Setup)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        uint256 hash = SignatureHash(scriptPubKey, spends[i], 0, SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT, 0, SIGVERSION_BASE);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT);
...
void ValidateCheckInputsForAllFlags(CMutableTransaction &tx, uint32_t failing_flags, bool add_to_cache, bool upgraded_nop)
{
    ...
    for (uint32_t test_flags=0; test_flags < (1U << 16); test_flags += 1) {
        CValidationState state;
        test_flags |= SCRIPT_ENABLE_SIGHASH_FORKID | SCRIPT_ENABLE_SIGHASH_FORKID_SHIFT;
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
    ...
    {
        std::vector<unsigned char> vchSig;
        uint256 hash = SignatureHash(p2pk_scriptPubKey, spend_tx, 0, SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT, 0, SIGVERSION_BASE);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
    ...
    CreateAndProcessBlock({spend_tx}, p2pk_scriptPubKey);
    //     BOOST_CHECK(chainActive.Tip()->GetBlockHash() == block.GetHash());
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        // Sign
        std::vector<unsigned char> vchSig;
        uint256 hash = SignatureHash(spend_tx.vout[2].scriptPubKey, invalid_with_cltv_tx, 0, SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT, 0, SIGVERSION_BASE);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT);
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        // Sign
        std::vector<unsigned char> vchSig;
        uint256 hash = SignatureHash(spend_tx.vout[3].scriptPubKey, invalid_with_csv_tx, 0, SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT, 0, SIGVERSION_BASE);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT);
    ...
    // for the same tx with a different witness.
    {
        ...
        ProduceSignature(MutableTransactionSignatureCreator(&keystore, &valid_with_witness_tx, 0, 11*CENT, SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT), spend_tx.vout[1].scriptPubKey, sigdata);
...

    {
        ...
        for (int i=0; i<2; ++i) {
            SignatureData sigdata;
            ProduceSignature(MutableTransactionSignatureCreator(&keystore, &tx, i, 11*CENT, SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT), spend_tx.vout[i].scriptPubKey, sigdata);

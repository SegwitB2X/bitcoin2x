...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s bitcoin2x-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  bitcoin2x-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded bitcoin2x transaction") + "\n" +
              "  bitcoin2x-tx [options] -create [commands]   " + _("Create hex-encoded bitcoin2x transaction") + "\n" +
              "\n";
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    bool fHashSingle = ((nHashType & ~(SIGHASH_ANYONECANPAY | SIGHASH_FORKID | SIGHASH_FORKID_SHIFT) ) == SIGHASH_SINGLE);
